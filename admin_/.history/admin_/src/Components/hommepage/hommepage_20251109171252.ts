...
@Component({
  selector: 'app-home',
  template: `
    <div class="p-4 sm:p-6 md:p-8 min-h-screen bg-gray-50">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
        <div
          class="bg-white rounded-xl shadow-md p-5 text-center hover:shadow-lg transition-shadow"
        >
          <h3 class="text-gray-600 font-medium mb-2">Total Users</h3>
          <p class="text-3xl font-bold text-blue-600">{{ totalUsers }}</p>
        </div>
        <div
          class="bg-white rounded-xl shadow-md p-5 text-center hover:shadow-lg transition-shadow"
        >
          <h3 class="text-gray-600 font-medium mb-2">Total Categories</h3>
          <p class="text-3xl font-bold text-orange-500">{{ totalCategories }}</p>
        </div>
        <div
          class="bg-white rounded-xl shadow-md p-5 text-center hover:shadow-lg transition-shadow"
        >
          <h3 class="text-gray-600 font-medium mb-2">Total Products</h3>
          <p class="text-3xl font-bold text-emerald-500">{{ totalProducts }}</p>
        </div>
      </div>

      <!-- Charts Grid -->
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 ml-12 mr-12">
        <div class="bg-white rounded-xl shadow-md p-4 h-96">
          <canvas #userChartCanvas class="h-full w-full"></canvas>
        </div>
        <div class="bg-white rounded-xl shadow-md p-4 h-96">
          <canvas #categoryChartCanvas class="h-full w-full"></canvas>
        </div>
        <div class="bg-white rounded-xl shadow-md p-4 h-96">
          <canvas #productChartCanvas class="h-full w-full"></canvas>
        </div>
      </div>
    </div>
  `,
