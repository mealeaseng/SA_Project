...
export class Navbar implements OnInit {
  ...
  getCategoryCount() {
    console.log('Calling getCategoryCount'); // Debug

    this.categoryService.getCategoryCount().subscribe({
      next: (response) => {
        console.log('Final response after mapping:', response); // Should be the number
      ...
      error: (error) => {
        ...
        this.http.get('http://localhost:3000/api/cetecory/count').subscribe({
          next: (rawResponse) => console.log('Raw API response:', rawResponse),
          error: (rawError) => console.error('Raw API error:', rawError),
        });
      },
    });
  }
